...
class Row(object):
    ...
    def __cmp__(self, other):
        ...
        return (self._cols > other._cols) - (self._cols < other._cols) or \
               (self._row > other._row) - (self._row < other._row)
...
class Rowset(object):
    ...
    def __str__(self):
        return "Rowset(columns=[{}], rows={})".format(','.join(self._cols),
                                                      len(self))
...
class IndexRowset(Rowset):
    ...
    def __init__(self, cols=None, rows=None, key=None):
        ...
        if self.composite:
            self._items = dict(
                (([row[k] for k in ki]), row) for row in self._rows
            )
        else:
...
class FilterRowset(object):
    ...
    def copy(self):
        return FilterRowset(self._cols[:], None, self.key, self.key2,
                            dict=copy.deepcopy(self._items))
    ...
    def __getitem__(self, i):
        if self.key2:
            return IndexRowset(self._cols, None, self.key2,
                               self._items.get(i, {}))
