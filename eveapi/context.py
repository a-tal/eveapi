...
from eveapi.exceptions import Error, ServerError, AuthenticationError
from eveapi.parser import _ParseXML
...
class _AuthContext(_Context):
    ...
    def character(self, characterID):
        ...
        return _Context(self._root, self._path + "/char", self.parameters,
                        {"characterID": characterID})

    def corporation(self, characterID):
        # same as character except for the folder "/corp"
        return _Context(self._root, self._path + "/corp", self.parameters,
                        {"characterID": characterID})
...
class _RootContext(_Context):
    ...
    def auth(self, **kw):
        if len(kw) == 2 and (("keyID" in kw and "vCode" in kw) or
           ("userID" in kw and "apiKey" in kw)):
    ...
    def setcachehandler(self, handler):
        if isinstance(handler, type):
            # late initialization of cache implementation
            handler = handler()
    ...
    def __call__(self, path, **kw):
        ...
        if response is None:
            if not USER_AGENT:
                warnings.warn(
                    "No User-Agent set! Please use the set_user_agent() "
                    "module-level function before accessing the EVE API.",
                    stacklevel=3,
                )
            ...
            if self._proxy is None:
                ...
                req = '{}://{}{}'.format(self._scheme, self._host, path)
                if self._proxySSL:
            ...
            if kw:
                conn.request("POST", req, urlencode(kw), {
            ...
            if response.status != 200:
                if response.status == httplib.NOT_FOUND:
                    raise AttributeError(
                        "'%s' not available (404 Not Found)" % path
                    )
                elif response.status == httplib.FORBIDDEN:
                    raise AuthenticationError(response.status,
                                              'HTTP 403 - Forbidden')
                else:
                    raise ServerError(
                        response.status,
                        "'%s' request failed (%s)" % (path, response.reason),
                    )
        ...
        fallback = cache and getattr(cache, "retrieve_fallback", False)
        if fallback:
            ...
            try:
                return _ParseXML(
                    response,
                    True,
                    store and (lambda obj: cache.store(
                        self._host,
                        path,
                        kw,
                        response,
                ...
                response = fallback(self._host, path, kw, reason=e)
