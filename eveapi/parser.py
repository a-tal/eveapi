...
from eveapi.containers import Element, Rowset, IndexRowset
from eveapi.exceptions import (
    AuthenticationError,
    Error,
    RequestError,
    ServerError,
...
def ParseXML(file_or_string):
    try:
        ...
        raise TypeError("XML data must string or file-like object")
...
def _ParseXML(response, fromContext, storeFunc):
    ...
    if fromContext and isinstance(response, Element):
        obj = response
    elif isinstance(response, STRING_TYPE):
        obj = _Parser().Parse(response, False)
    elif hasattr(response, "read"):
        obj = _Parser().Parse(response, True)
    else:
        raise TypeError(
            "response must be None, str, file-like or Element, not {}".format(
                type(response)
            )
        )
...
class _Parser(object):
    ...
    def tag_start(self, name, attributes):
        ...
        if name == "rowset":
            ...
            try:
                columns = attributes[attributes.index('columns') + 1].replace(
                    " ", "").split(",")
            except ValueError:
        ...
        if isinstance(self.container, Rowset) and \
           self.container.__catch == this._name:
            # <hack>
            ...
            attrs = len(attributes) / 2
            cols = len(self.container._cols)
            if attrs < cols and (attributes[-2] == self.container._cols[-1]):
                # the row data is missing attributes that were defined in the
                # rowset. missing attributes' values will be set to None.
                fixed = []
                row_idx = 0
                hdr_idx = 0
                attrs *= 2
                ...
                if not self.container._cols or (attrs > cols):
                    # the row data contains more attributes than were defined.
                    self.container._cols = attributes[0::2]
                self.container.append([
                    _castfunc(attributes[i], attributes[i + 1]) for
                    i in range(0, len(attributes), 2)
                ])
            # </hack>
    ...
    def tag_end(self, name):
        ...
        if self.container._isrow:
            ...
            # see if there's already an attribute with this name (this
            # shouldn't really happen, but it doesn't hurt to handle this case!
            sibling = getattr(self.container, this._name, None)
            if sibling is None:
                if not self.has_cdata and self._last is this and \
                   name != "rowset":
                    if attributes:
            ...
            # multiples of some tag or attribute. Code below handles this case.
                ...
                rs.append(row)
                row = [
                    getattr(sibling, attributes[i]) for
                    i in range(0, len(attributes), 2)
                ] + [getattr(sibling, col) for col in attributes2]
                rs.append(row)
                rs._cols = [
                    attributes[i] for i in range(0, len(attributes), 2)
                ] + [col for col in attributes2]
        ...
        for i in range(0, len(attributes), 2):
            this.__dict__[attributes[i]] = _castfunc(
